function maxScore(nums1: number[], nums2: number[], k: number): number {
    const combined = nums2.map((n, i) => [n, nums1[i]]);
    combined.sort((a, b) => b[0] - a[0]);
    const maxElems = new MyPriorityQueue<number>();
    let maxSum = 0;
    for (let i = 0; i < k; i++) {
        maxElems.add(combined[i][1]);
        maxSum += combined[i][1];
    }
    let maximumScore = maxSum * combined[k - 1][0];
    for (let i = k; i < nums1.length; i++) {
        maxElems.add(combined[i][1]);
        maxSum += (combined[i][1] - maxElems.poll());
        maximumScore = Math.max(maximumScore, maxSum * combined[i][0]);
    }
    return maximumScore;
}
